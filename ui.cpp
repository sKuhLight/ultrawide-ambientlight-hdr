...
bool RenderUI(AppSettings& settings, UINT gameWidth, UINT gameHeight)
{
    ...
    if (ImGui::CollapsingHeader("Effects", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        {
            if (ImGui::SliderFloat("Saturation", &settings.saturation, 0.0f, 3.0f, "%.2f"))
            {
                SaveSettings(settings);
            }

            if (ImGui::SliderFloat("Brightness", &settings.brightness, 0.1f, 3.0f, "%.2f"))
            {
                SaveSettings(settings);
            }

            if (ImGui::SliderFloat("Contrast", &settings.contrast, 0.1f, 3.0f, "%.2f"))
            {
                SaveSettings(settings);
            }

            if (ImGui::Checkbox("HDR Boost", &settings.hdrBoostEnabled))
            {
                SaveSettings(settings);
            }

            if (settings.hdrBoostEnabled)
            {
                if (ImGui::SliderFloat("HDR Strength", &settings.hdrBoostStrength, 0.0f, 2.0f, "%.2f"))
                {
                    SaveSettings(settings);
                }
            }

            if (ImGui::Checkbox("True HDR Output", &settings.hdrOutputEnabled))
            {
                SaveSettings(settings);
            }
            if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
            {
                ImGui::SetTooltip("Uses FP16/scRGB swapchain for real HDR signal output. Requires Windows HDR enabled. Restart app after changing.");
            }

            if (settings.hdrOutputEnabled)
            {
                if (ImGui::SliderFloat("HDR Paper White (nits)", &settings.hdrPaperWhiteNits, 80.0f, 1000.0f, "%.0f"))
                {
                    SaveSettings(settings);
                }
            }
