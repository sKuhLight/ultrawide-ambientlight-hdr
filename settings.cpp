...
bool ReadSettings(AppSettings& settings)
{
    ...
    inipp::get_value(ini.sections["Game"], "Saturation", saturation);

    float brightness = DEFAULT_BRIGHTNESS;
    inipp::get_value(ini.sections["Game"], "Brightness", brightness);

    float contrast = DEFAULT_CONTRAST;
    inipp::get_value(ini.sections["Game"], "Contrast", contrast);

    bool hdrBoostEnabled = DEFAULT_HDR_BOOST_ENABLED;
    inipp::get_value(ini.sections["Game"], "HdrBoostEnabled", hdrBoostEnabled);

    float hdrBoostStrength = DEFAULT_HDR_BOOST_STRENGTH;
    inipp::get_value(ini.sections["Game"], "HdrBoostStrength", hdrBoostStrength);

    bool hdrOutputEnabled = DEFAULT_HDR_OUTPUT_ENABLED;
    inipp::get_value(ini.sections["Game"], "HdrOutputEnabled", hdrOutputEnabled);

    float hdrPaperWhiteNits = DEFAULT_HDR_PAPER_WHITE_NITS;
    inipp::get_value(ini.sections["Game"], "HdrPaperWhiteNits", hdrPaperWhiteNits);
    ...
    settings.hdrBoostEnabled = hdrBoostEnabled;
    settings.hdrBoostStrength = hdrBoostStrength;
    settings.hdrOutputEnabled = hdrOutputEnabled;
    settings.hdrPaperWhiteNits = hdrPaperWhiteNits;
...
void SaveSettings(AppSettings& settings)
{
    ...
    ini.sections["Game"]["Saturation"] = std::to_string(settings.saturation);
    ini.sections["Game"]["Brightness"] = std::to_string(settings.brightness);
    ini.sections["Game"]["Contrast"] = std::to_string(settings.contrast);
    ini.sections["Game"]["HdrBoostEnabled"] = settings.hdrBoostEnabled ? "true" : "false";
    ini.sections["Game"]["HdrBoostStrength"] = std::to_string(settings.hdrBoostStrength);
    ini.sections["Game"]["HdrOutputEnabled"] = settings.hdrOutputEnabled ? "true" : "false";
    ini.sections["Game"]["HdrPaperWhiteNits"] = std::to_string(settings.hdrPaperWhiteNits);
